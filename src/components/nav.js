...
const Nav = ({ isHome }) => {
  ...
  const ResumeLink = (
    <a className="resume-button" href="/Ahmed_Mribai_Resume_Advanced.pdf" target="_blank" rel="noopener noreferrer">
      Resume
    </a>
  );
              ...
              <ol>
                <TransitionGroup component={null}>
                  {isMounted &&
                    navLinks &&
                    navLinks.map(({ url, name }, i) => {
                      if (!linkRefs.current[i]) linkRefs.current[i] = React.createRef();
                      return (
                        <CSSTransition
                          key={i}
